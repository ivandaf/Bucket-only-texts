...
//SourceBucketRating: array [1..100] of integer; // Ретинг Источников для сортировки и выполнения в порядке понижения рейтинга
  ...
  kon : integer;  // хранит номер последнего непустого эл-та в массиве ArrSourceClasses
...
var
  ...
  MatrixDimensionFile : TextFile; // Количества столбцов и строк в матрице отношений - файл
  MatrixDimensionFileLocation : String; // Имя, адрес файла
  ClassNameFile : TextFile;  // названия классов
  ClassNameFileLocation : String;  // Имя, адрес файла
  DisjointMatrixFile : TextFile; // Матрица отношений - файл
  DisjointMatrixFileLocation : String; // Имя, адрес файла
begin
 najal:=0;
 najal2raza:=0;
 ////////////////////////////// ## DUMIN ## //////////////////////////////
...
// ВВОД: ОПИСАНИЕ ВХОДНЫХ ДАННЫХ
     ...
     /////////////////////////// ## ABRAMOV ## ////////////////////////////////
      AssignFile(ClassNameFile,ClassNameFileLocation);    // читаем названия имеющихся полей из файла    вместо явного присваивания
      Reset(ClassNameFile);  // Имена классов - список
      indmas:=1;
...
/////////////////////////// ## ABRAMOV ## ////////////////////////////////
 ...
 ClassesMatrix.ColCount:= StrToInt(coltabl);
      ...
      While not Eof(DisjointMatrixFile) do begin
        for i:=0 to High(ArrDisjointClasses) do begin  // массив начинается с 0
           ReadLn(DisjointMatrixFile,tablstr);
           for j:=0 to (Length(tablstr))-1 do begin
               ArrDisjointClasses[i,j] := StrToInt(tablstr[j+1]);   // массив начинается с 0
               ClassesMatrix.Cells[j+1,i+1] := tablstr[j+1];         // строка начинается с 1
           end;
           tablstr:='';
        end;
      end;
